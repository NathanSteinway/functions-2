...
let gooba = (a, b) => {
    console.log("The first parameter is " + a + ". The second parameter is" + b + ".")
}

console.log(gooba(5, ' haha'))
...
let greeting = function(firstName, lastName, greetingCallback) {
    greetingCallback(firstName + lastName)
}

let greetingCallback = function(fullName) {
    return console.log('Hello, my full name is ' + fullName)
}

greeting('Nathan ', 'Steinway', greetingCallback)
...
let pricesPlusTax = function(prices, pricesCallback) {
    for (let i = 0; i < prices.length; i++){
        totalCost.push(prices[i] + (.25 * prices[i]))
    }
    pricesCallback(totalCost)
}

let pricesCallback = function(totalCost) {
...
pricesPlusTax(prices, pricesCallback)
